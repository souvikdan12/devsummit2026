const express = require('express');
const router = express.Router();
const { generateJSON } = require('../services/geminiService');
const prompts = require('../services/promptBuilder');

// POST /api/compare - Resume vs requirements comparison (Feature 5)
router.post('/', async (req, res) => {
  try {
    const { resume_text, desired_role, target_company } = req.body;

    if (!resume_text || !desired_role) {
      return res.status(400).json({ error: 'resume_text and desired_role are required' });
    }

    const prompt = prompts.resumeComparison(resume_text, desired_role, target_company || '');
    const result = await generateJSON(prompt);

    res.json({ success: true, data: result });
  } catch (error) {
    console.error('Comparison error:', error);
    res.status(500).json({ error: 'Comparison failed: ' + error.message });
  }
});

module.exports = router;
